...
bool process_autocorrect(uint16_t keycode, keyrecord_t* record) {
    static uint8_t typo_buffer[DICTIONARY_MAX_LENGTH];
    static uint8_t buffer_size = 0;

    // Reset with non-Shift modifiers or when feature is off.
    if (get_mods() & ~MOD_MASK_SHIFT || !autocorrect_on) {
        buffer_size = 0;
        return true;
    }
    ...
    if (keycode > KC_Z) {
        if (KC_TAB <= keycode && keycode <= KC_SLASH) {
            ...
            buffer_size = 0;
            keycode     = KC_SPC;
        } else if (keycode == KC_BSPC && buffer_size > 0) {
    ...
    typo_buffer[buffer_size++] = keycode;
    ...
    for (int8_t i = buffer_size - 1; i >= 0; --i) {
        if (code & 64) {  // Check for match in node with multiple children.
            code &= 63;
            while (code != typo_buffer[i]) {
                if (!code) return true; // Not found
                code = pgm_read_byte(dictionary + (state += 3));
            }
        ...
        // Safe guard against invalid state index
        ...
        if (code & 128) {  // A typo was found! Apply correction.
            ...
            for (uint8_t j = 0; j < backspaces; ++j) tap_code(KC_BSPC);
