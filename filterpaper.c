...
#define IS_QUICK_SUCCESSION_INPUT(k, r, ctx) (                    \
    ...
    && last_matrix_activity_elapsed() <= SHORT_TAP_TERM           \
)

// Checks for an active modifier matching the opposite hand
// alpha row mod-tap key press after TAPPING_TERM interval
#define IS_BILATERAL_MOD_INPUT(k, r) (                  \
    get_mods() & (((k) & 0x1000) ? 0x0F : 0xF0)         \
    && IS_QK_MOD_TAP((k)) && IS_ALPHA_ROW((r))          \
    && last_matrix_activity_elapsed() >= TAPPING_TERM   \
)
...
static bool tapped[QK_BASIC_MAX + 1];
...
bool pre_process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        ...
        if (IS_QUICK_SUCCESSION_INPUT(keycode, record, context)
            || IS_BILATERAL_MOD_INPUT(keycode, record)) {
...
uint16_t get_tapping_term(uint16_t keycode, keyrecord_t *record) {
    return IS_SHIFT_TAP(keycode) && IS_ALPHA_ROW(record) ? SHORT_TAP_TERM : TAPPING_TERM;
}
