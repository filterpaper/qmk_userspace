...
#define IS_HOMEROW_CAG(k, r) (IS_HOMEROW(r) && !IS_MOD_TAP_SHIFT(k))

#define IS_TYPING(k) ( \
    (GET_TAP_KEYCODE(k) <= KC_Z || IS_SHORTCUT(k)) && \
    (last_input_activity_elapsed() < INPUT_IDLE_MS))

// Same-hand key press detection using row bitmasks
// Homerow 1 yields 0x07 (0b00000111) to match any of rows 0,1,2
// Homerow 5 yields 0x70 (0b01110000) to match any of rows 4,5,6
#define UNILATERAL_MASK(h) ((h) == 1 ? 0x07 : ((h) == 5 ? 0x70 : 0x00))
#define IS_UNILATERAL(r, n) (UNILATERAL_MASK((r)->event.key.row) & (1U << (n).event.key.row))
#define IS_UNILATERAL_CAG(r, c) (IS_UNILATERAL((r), (c).record) && !IS_MOD_TAP_SHIFT((c).keycode))
...
bool get_hold_on_other_key_press(uint16_t keycode, keyrecord_t *record) {
    ...
    if (IS_UNILATERAL_CAG(record, context) || IS_SHORTCUT(keycode)) {
